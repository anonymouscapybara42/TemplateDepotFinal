...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // process.env is read live by Node at request time — the correct
    // source for secrets configured on the host (Hostinger's dashboard,
    // a real .env file, etc). import.meta.env is only kept as a fallback
    // for local `astro dev`, and must never take precedence: if a value
    // ever got baked in at build time it would permanently override
    // whatever is actually set on the server, which is never what you want
    // for credentials.
    const SMTP_HOST = process.env.SMTP_HOST || import.meta.env.SMTP_HOST || 'smtp.gmail.com';
    ...
    const INQUIRY_TO_EMAIL =
      process.env.INQUIRY_TO_EMAIL || import.meta.env.INQUIRY_TO_EMAIL || SMTP_USER;
